...
def predict():
    ...
    messagebox.showinfo("السعر المتوقع",f"The price of the land is {my_lands_price:,.2f} SAR ,for land located in a neighborhood {district}\n"
                                         f"Current price per meter {my_lands.price_meter():,.2f} SAR\n"
                                        f"The expected future price per meter {my_lands.future_price_meter():,.2f} SAR")
...
def predict1():
    ...
    my_lands = lands(district, area, streets)
    my_lands.price_meter()
    messagebox.showinfo("السعر المستقبلي المتوقع للمتر",f"The expected future price per meter {my_lands.future_price_meter():,.2f} SAR")

window1 = Tk()
window1.title("توقع اسعار الاراضي")
window1.geometry("1200x630+50+50")
photo = PhotoImage(file="img/11.png")
labg = Label(window1,image=photo)
labg.place(x=0,y=0,relwidth=1,relheight=1)
la = Label(window1,text=" Welcome to the land price calculator program ")
la.grid(padx=10, pady=5)
la1 = Label(window1,text="Enter the district name :")
la1.grid(row=1, column=0, padx=10, pady=5)
com1 = ttk.Combobox(window1,values=(my_quarter1),state="readonly")
com1.grid(row=1, column=1, padx=10, pady=5)

la2 =Label(window1,text="Enter the land area : ")
la2.grid(row=2, column=0, padx=10, pady=5)
en2=Entry(window1,width=25)
en2.grid(row=2, column=1, padx=10, pady=5)

la3 = Label(window1,text="Enter the number of streets : ")
la3.grid(row=3, column=0, padx=10, pady=5)
com2 =  ttk.Combobox(window1,values=("1","2","3","4"),state="readonly")
com2.grid(row=3, column=1, padx=10, pady=5)

la4 = Label(window1,text="Enter the number of streets : ")
la4.grid(row=4, column=0, padx=10, pady=5)
com3 =  ttk.Combobox(window1,values=("1","2","3","4","5"),state="readonly")
com3.grid(row=4, column=1, padx=10, pady=5)

bu1 = Button(window1,text="حساب السعر", command=predict)
bu1.grid(row=5, column=1, padx=10, pady=5)
bu2 = Button(window1,text="سعر المتر المتوقع", command=predict1)
